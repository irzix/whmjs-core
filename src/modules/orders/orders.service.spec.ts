...
describe('OrdersService - checkout()', () => {
  ...
  const makeItems = (count: number) => Array.from({ length: count }, (_, i) => ({ id: i + 1, cartId: 10, productId: i + 1, quantity: 1, config: null }));
  const makeProducts = (count: number) => Array.from({ length: count }, (_, i) => ({ id: i + 1, price: 100, currencyId: 1, isActive: true }));

  beforeEach(async () => {
    prismaMock = {
      organization: { findUnique: jest.fn().mockResolvedValue({ id: 1, currencyId: 1 }) },
      cart: { update: jest.fn().mockResolvedValue({ id: 10, status: 'PROCESSING' }) },
      cartItem: { findMany: jest.fn().mockResolvedValue(makeItems(3)), deleteMany: jest.fn() },
      ...
      invoice: { create: jest.fn().mockResolvedValue({ id: 200, status: 'PENDING', total: 327, currencyId: 1 }) },
      transaction: { create: jest.fn().mockResolvedValue({ id: 300 }) },
      $transaction: jest.fn().mockImplementation((cb) => cb(prismaMock)),
    };

    currencyConverterMock = {
      convert: jest.fn((items) => Promise.resolve(items.map(i => ({ id: i.id, amount: i.amount }))))
    };

    couponsCalculatorMock = { calculateDiscount: jest.fn().mockResolvedValue(0) };

    const module: TestingModule = await Test.createTestingModule({
      providers: [
        OrdersService,
        { provide: PrismaService, useValue: prismaMock },
        { provide: PaymentGatewaysHandler, useValue: { create: jest.fn().mockResolvedValue({ url: 'http://pay.me' }) } },
  ...
  describe('Core Logic', () => {
    ...
    it('should throw if products are missing or inactive', async () => {
      prismaMock.product.findMany.mockResolvedValue([makeProducts(1)[0], { id: 2, isActive: false }]);
      await expect(service.checkout(baseDto, currentUser)).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('Performance & Concurrency', () => {
    ...
    it('should handle stress load and prevent race conditions', async () => {
      ...
      const tasks = Array.from({ length: 100 }, () => service.checkout(baseDto, currentUser));
      const start = performance.now();
      ...
      prismaMock.cart.update.mockImplementation(async (params) => {
        if (params.data.status === 'PROCESSING' && ++procCount > 1) throw new Error('Lock Error');
        return {};
      });

      const results = await Promise.allSettled([service.checkout(baseDto, currentUser), service.checkout(baseDto, currentUser)]);
